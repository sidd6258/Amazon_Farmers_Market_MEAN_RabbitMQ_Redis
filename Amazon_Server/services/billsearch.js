...
function createBill(billingid,deliverydate,productid,amount,source_add_prod,source_city_prod,source_zipcode_prod,dest_add,dest_city,dest_zip,custid,qty,availableDriverId){
	for(i=0;i<productid.length;i++){
		...
		var query3= "insert into billing(`BILLING_ID`,`EXPCTD_DELIVERY_DATE`,`PRODUCT_ID`,`TOTAL_AMOUNT`,`SOURCE_ADDRESS`,`SOURCE_CITY`,`SOURCE_ZIP`,`DESTINATION_ADDRESS`,`DESTINATION_CITY`,`DESTINATION_ZIP`,`DRIVER_ID`,`CUSTOMER_ID`,`QUANTITY`) " +
				"values ('"+billingid+"','"+deliverydate+"', '"+productid[i]+"','"+amount[i]+"','"+source_add_prod[i]+"','"+source_city_prod[i]+"','"+source_zipcode_prod[i]+"','"+dest_add+"','"+dest_city+"','"+dest_zip+"',"+availableDriverId+",'"+custid+"','"+qty[i]+"')";
		mysql.insertData1(function(err, results){
			if (err) 
			{
				throw err;
...
function fetchBillid(noOfFarmers,deliverydate,source_add,source_city,source_zip,productid,driver_id,truck_id,amount,source_add_prod,source_city_prod,source_zipcode_prod,dest_add,dest_city,dest_zip,custid,qty){
	...
	mysql.fetchData(function(err, results){
		if (err) 
		{
			...
			var query7= "Select DRIVER_ID  from TRUCKS where STATUS='Available' LIMIT 1";
			mysql.fetchData(function(err, results){
				if (err) 
				{
					throw err;
					...
					if(results.length > 0)
						{
						availableDriverId=results[0].DRIVER_ID;
						createBill(billingID,deliverydate,productid,amount,source_add_prod,source_city_prod,source_zipcode_prod,dest_add,dest_city,dest_zip,custid,qty,availableDriverId);
